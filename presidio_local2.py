...
class Pipeline:
    ...
    def __init__(self):
        self.type = "filter"
        self.name = "Presidio PII Redaction (Offline) 2 fixed"
    ...
    def fix_numpy_compatibility(self):
        """Фиксит проблему совместимости numpy"""
        try:
            import numpy
            print(f"📊 Текущая версия numpy: {numpy.__version__}")
            
            # Если проблема с numpy, переустанавливаем с правильной версией
            if hasattr(numpy, '__version__') and numpy.__version__.startswith('1.'):
                print("🔄 Переустановка numpy для совместимости...")
                subprocess.check_call([
                    sys.executable, "-m", "pip", "install", "--upgrade",
                    "--no-index", "--find-links", self.wheels_dir,
                    "numpy==1.24.3"
                ])
                print("✅ numpy обновлен")
                
        except Exception as e:
            print(f"⚠️ Ошибка при проверке numpy: {e}")

    def install_dependencies(self):
        """Устанавливает все зависимости из локальных wheel-файлов с правильным порядком"""
        try:
            ...
            # 1. Сначала устанавливаем numpy правильной версии
            print("📦 Установка numpy...")
            numpy_wheels = [f for f in os.listdir(self.wheels_dir) if f.startswith('numpy-')]
            if numpy_wheels:
                numpy_wheel = os.path.join(self.wheels_dir, numpy_wheels[0])
                subprocess.check_call([sys.executable, "-m", "pip", "install", "--force-reinstall", numpy_wheel])
            
            # 2. Устанавливаем остальные базовые зависимости
            print("📦 Установка базовых зависимостей...")
            base_packages = ["cython", "wheel", "setuptools"]
            for package in base_packages:
                package_wheels = [f for f in os.listdir(self.wheels_dir) if f.startswith(package)]
                if package_wheels:
                    package_wheel = os.path.join(self.wheels_dir, package_wheels[0])
                    subprocess.check_call([sys.executable, "-m", "pip", "install", package_wheel])
            
            # 3. Устанавливаем spaCy модель
            print(f"📦 Установка spaCy модели из: {self.spacy_wheel}")
            subprocess.check_call([sys.executable, "-m", "pip", "install", self.spacy_wheel])
            
            # 4. Устанавливаем thinc (зависимость spaCy)
            thinc_wheels = [f for f in os.listdir(self.wheels_dir) if f.startswith('thinc-')]
            if thinc_wheels:
                thinc_wheel = os.path.join(self.wheels_dir, thinc_wheels[0])
                subprocess.check_call([sys.executable, "-m", "pip", "install", thinc_wheel])
            
            # 5. Устанавливаем Presidio
    ...
    async def on_startup(self):
        ...
        try:
            ...
            try:
                import spacy
                print(f"🔍 Загрузка spaCy модели {self.valves.spacy_model_name}...")
