...
class Pipeline:
    ...
    def __init__(self):
        self.type = "filter"
        self.id = "chunking4 llmguard"
        self.name = "chunking4 llmguard"
        
        self.valves = self.Valves()
        
        # Инициализируем атрибуты в __init__
        self.model = None
        self.tokenizer = None
    ...
    async def on_startup(self):
        try:
            ...
            self.model_loaded = False
            # Убеждаемся что атрибуты существуют даже при ошибке
    ...
    def predict_chunked(self, text: str):
        ...
        if self.valves.check_sentences:
            ...
            for i, sentence in enumerate(sentences):
                ...
                if risk_sent > self.valves.threshold * 0.8:
    ...
    async def inlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        # Если безопасно или блокировка отключена
        if is_safe:
            print("\n✅ Сообщение безопасно, пропускаем")
        else:
            print(f"\n⚠️ Обнаружена инжекция, но блокировка отключена")
    ...
    async def on_shutdown(self):
        ...
        print("🔄 Выключение пайплайна...")
        
        # Проверяем наличие модели перед удалением
        if hasattr(self, 'model') and self.model is not None:
            del self.model
            print("🧹 Модель выгружена")
        
        # Очищаем CUDA кэш если нужно
        if hasattr(self, 'device') and self.device.type == 'cuda' and torch.cuda.is_available():
            torch.cuda.empty_cache()
            print("🧹 CUDA кэш очищен")
        
        # Явно удаляем другие атрибуты
        if hasattr(self, 'tokenizer') and self.tokenizer is not None:
            del self.tokenizer
        
        print("✅ Пайплайн успешно выключен")
